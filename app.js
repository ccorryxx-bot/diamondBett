...
function openDepositModal(){
  ...
  const step1 = document.getElementById('depStep1');
  const step2 = document.getElementById('depStep2');
  const balShow = document.getElementById('depBalShow');

  if(step1) step1.style.display = 'block';
  if(step2) step2.style.display = 'none';
  if(balShow) balShow.textContent = document.getElementById('statBalance')?.textContent || '0.00';

  fetchDepMethods(supabase);
}

async function fetchDepMethods(db = supabase){
...
function pickBonus(yes){
  _dBonus = yes;
  const yesEl = document.getElementById('bOptYes');
  const noEl = document.getElementById('bOptNo');
  const preview = document.getElementById('depPreview');
  if(yesEl) yesEl.classList.toggle('selected', yes);
  if(noEl) noEl.classList.toggle('selected', !yes);
  if(preview) preview.style.display = yes ? 'block' : 'none';
  updatePreview();
}
...
function goStep2(){
  ...
  const dep2Logo = document.getElementById('dep2Logo');
  if(dep2Logo) dep2Logo.innerHTML = getProvSvg(_dMethod.provider_name, 40);
...
function cpText(id){
  const el = document.getElementById(id);
  if(!el) return;
  navigator.clipboard.writeText(el.textContent).then(() => gToast('ကော်ပီ ✅'));
}
...
async function submitSlip(db = supabase){
  const boxes = document.querySelectorAll('.slip-box');
...
function switchWdTab(tab, el){
  ...
  if(tab === 'hist') loadTxHistory(supabase);
}
...
function renderLinked(){
  ...
  if(hasAcct) hasAcct.style.display = 'block';

  const logo = document.getElementById('wdLinkedLogo');
  if(logo) logo.innerHTML = getProvSvg(_linked.provider, 40);
...
function updateLinkTab(){
  ...
  if(item && !item.querySelector('.acct-linked-badge')){
...
function openSheet(prov){
  ...
  const nameInp = document.getElementById('lnkName');
  const numInp = document.getElementById('lnkNum');
...
async function confirmLink(db = supabase){
...
// WITHDRAW REQUEST
// ============================================================
async function doWithdraw(db = supabase){
  ...
  try{
    const [uRes, sRes] = await Promise.all([
      db.from('users').select('balance,remaining_turnover').eq('id', currentUserId).single(),
      db.from('site_settings').select('min_withdrawal,max_withdrawal').eq('id', 1).single()
    ]);
    ...
    if(tv > 0){
      ...
      const bar = document.getElementById('wdTvBar');
    ...
    if(amount < min){
      gToast(`❌ အနည်းဆုံး ${min.toLocaleString()} ကျပ်`);
      resetWdBtn();
      return;
    }

    if(amount > max){
      gToast(`❌ အများဆုံး ${max.toLocaleString()} ကျပ်`);
      resetWdBtn();
      return;
    }

    if(amount > bal){
      gToast('❌ Balance မလုံလောက်ပါ');
      resetWdBtn();
      return;
    }
...
// TRANSACTION HISTORY
// ============================================================
async function loadTxHistory(db = supabase){
...
document.addEventListener("DOMContentLoaded", async () => {
  // Supabase init — wait until CDN is ready
  if(!window.supabase){
    console.error('Supabase CDN not loaded');
    gToast('Supabase library load မပြီးသေးပါ');
    return;
  }
  ...
  const homeBtn = document.querySelector('.bnav-btn[data-nav="home"]');
  ...
  // Games
  async function loadGamesFromDB(db = supabase){
    const { data: games, error } = await db.from('games').select('*');
  ...
  await loadGamesFromDB(supabase);
  ...
  // Dashboard stats
  async function loadDashboardStats(userId, db = supabase){
    const { data, error } = await db.from('agent_dashboard_stats')
  ...
  // My Data tab
  async function loadMyData(agentId, period = 'today', db = supabase){
  ...
  document.getElementById('timePills')?.addEventListener('click', e => {
    ...
    loadMyData(currentAgentId, pill.dataset.period, supabase);
  });
  ...
  const closeRole = () => { document.getElementById('dlRoleDropdown')?.style && (document.getElementById('dlRoleDropdown').style.display = 'none'); };
  ...
  (function(){
    ...
    ['dlStartMonth','dlEndMonth'].forEach(id => {
      ...
      for(let mo = 1; mo <= 12; mo++){
        const o = document.createElement('option');
        o.value = String(mo).padStart(2,'0');
        o.textContent = String(mo).padStart(2,'0');
    ...
    ['dlStartDay','dlEndDay'].forEach(id => {
      ...
      for(let dy = 1; dy <= 31; dy++){
        const o = document.createElement('option');
        o.value = String(dy).padStart(2,'0');
        o.textContent = String(dy).padStart(2,'0');
        if(dy === d) o.selected = true;
        s.appendChild(o);
      }
    });
  })();
  ...
  document.getElementById('dlRoleDropdown')?.querySelectorAll('.dl-role-option').forEach(opt => {
    opt.addEventListener('click', () => {
      ...
      closeRole();
      loadDownline(undefined, supabase);
    });
  });
  ...
  document.getElementById('dlSearchSubmit')?.addEventListener('click', () => {
    loadDownline(document.getElementById('dlSearchInput')?.value.trim(), supabase);
  });

  async function loadDownline(searchId = '', db = supabase){
    if(!currentAgentId) return;

    const { data, error } = await db.rpc('get_agent_subordinates', { p_agent_id: currentAgentId });
  ...
  document.getElementById('agentTabBar')?.addEventListener('click', e => {
    ...
    if(btn.dataset.atab === 'mydata' && currentAgentId){
      ...
      loadMyData(currentAgentId, p, supabase);
    }
    if(btn.dataset.atab === 'downline' && currentAgentId) loadDownline('', supabase);
  });
  ...
  document.getElementById('registerBtn')?.addEventListener('click', async () => {
    ...
    try{
      ...
      if(resp.ok){
        gToast("မှတ်ပုံတင်ခြင်း အောင်မြင်သည်! Code: " + result.ref_code, 'success');
      ...
      console.error(err);
      gToast("Edge Function နဲ့ ချိတ်ဆက်လို့ မရပါ");
    }
  });
  ...
  function onLoginSuccess(user, refCode, balance = 0, userId = null){
    ...
    document.getElementById('showAuthBtn')?.style && (document.getElementById('showAuthBtn').style.display = 'none');
    document.getElementById('walletBtns')?.style && (document.getElementById('walletBtns').style.display = 'flex');
    ...
    const shareInp = document.getElementById('agentShareLinkInput');
    ...
    document.getElementById('agentLocked')?.style && (document.getElementById('agentLocked').style.display = 'none');
    document.getElementById('agentUnlocked')?.style && (document.getElementById('agentUnlocked').style.display = 'flex');
    ...
    if(currentUserId){
      loadDashboardStats(currentUserId, supabase);
      initLinked();
    }
  }
  ...
  document.getElementById('copyPhoneBtn')?.addEventListener('click', () => {
    navigator.clipboard.writeText(document.getElementById('agentPhoneDisplay')?.textContent || '').then(() => gToast("ကူးယူပြီးပါပြီ! ✅"));
  });
  ...
  function copyAgentLink(){
    ...
    navigator.clipboard.writeText(input.value).then(() => gToast("Link ကူးယူပြီးပါပြီ! ✅")).catch(() => {
      input.select();
