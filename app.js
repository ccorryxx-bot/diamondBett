...
function shareVia(platform) {
  ...
  const urls = { telegram:`https://t.me/share/url?url=${encodeURIComponent(link)}&text=${text}`, viber:`viber://forward?text=${text}`, facebook:`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(link)}`, whatsapp:`https://wa.me/?text=${text}` };
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  (function(){
    ...
    wrap.addEventListener("touchend",e=>{const diff=sx-e.changedTouches[0].clientX;if(Math.abs(diff)>40)go(diff>0?cur+1:cur-1);start();},{passive:true});
    update(); start();
  })();
  ...
  // ============================================================
  const wheelSlots = [
    {label:'5,000',  amount:5000,  color:'#6B1010',rimColor:'#8B1515'},
    {label:'15,000', amount:15000, color:'#3D0707',rimColor:'#5A0D0D'},
    {label:'30,000', amount:30000, color:'#6B1010',rimColor:'#8B1515'},
    {label:'50,000', amount:50000, color:'#3D0707',rimColor:'#5A0D0D'},
    {label:'65,000', amount:65000, color:'#6B1010',rimColor:'#8B1515'},
    {label:'80,000', amount:80000, color:'#3D0707',rimColor:'#5A0D0D'},
    {label:'ဗလာ',   amount:0,     color:'#151525',rimColor:'#1E1E35'},
    {label:'ဗလာ',   amount:0,     color:'#0D0D18',rimColor:'#141420'},
  ];
  ...
  function drawWheel(angle) {
    ...
    grd.addColorStop(0,'rgba(201,162,39,0.6)');
    ...
    wheelSlots.forEach((slot,i) => {
      ...
      // Text
      ctx.save();
      ctx.translate(cx,cy); ctx.rotate(start + sa/2);
      const isBlank = slot.amount === 0;
      ctx.fillStyle = isBlank ? '#333' : '#FFD700';
      ctx.font = `bold ${isBlank ? '11' : '10'}px "Segoe UI",sans-serif`;
    ...
    const cGrd = ctx.createRadialGradient(cx-5,cy-5,3,cx,cy,22);
    cGrd.addColorStop(0,'#FFE55C'); cGrd.addColorStop(1,'#8B6014');
  ...
  function spinToSlot(slotIndex, onDone) {
    ...
    // Target: slot idx center at top (-π/2)
    // Slot idx center at 0-rotation = idx*sa + sa/2 (from 0=right, clockwise)
    // When wheel rotates by `angle`, slot appears at: idx*sa + sa/2 + angle = 3π/2 (top)
    // angle_target = 3π/2 - idx*sa - sa/2
    ...
    const totalSpin = 6 * Math.PI*2 + diff;
    const endAngle  = wheelAngle + totalSpin;
    const duration  = 5000;
  ...
  document.getElementById('dlDateConfirm').addEventListener('click', () => {
    const activePill = dlDateModal.querySelector('.dl-period-btn.active');
    if (activePill) {
      dlCurrentPeriod = activePill.dataset.period;
      setEl('dlDateLabel', activePill.textContent);
    }
    closeDateModal();
    loadDownline();
  });
  ...
  function populateDateSelects() {
    ...
    ['dlStartMonth','dlEndMonth'].forEach(id => {
      ...
      for (let mo = 1; mo <= 12; mo++) {
        const opt = document.createElement('option');
        opt.value = String(mo).padStart(2,'0'); opt.textContent = String(mo).padStart(2,'0');
        if (mo === m) opt.selected = true;
        sel.appendChild(opt);
      }
    });
    ['dlStartDay','dlEndDay'].forEach(id => {
      ...
      for (let dy = 1; dy <= 31; dy++) {
        const opt = document.createElement('option');
        opt.value = String(dy).padStart(2,'0'); opt.textContent = String(dy).padStart(2,'0');
        if (dy === d) opt.selected = true;
        sel.appendChild(opt);
      }
    });
  }
  ...
  function closeRoleDropdown() { dlRoleDropdown.style.display = 'none'; }

  document.getElementById('dlRoleBtn').addEventListener('click', e => {
    e.stopPropagation();
    dlRoleDropdown.style.display = dlRoleDropdown.style.display === 'block' ? 'none' : 'block';
  });
  ...
  dlRoleDropdown.querySelectorAll('.dl-role-option').forEach(opt => {
    opt.addEventListener('click', () => {
      ...
      dlCurrentSort = opt.dataset.sort;
      setEl('dlRoleLabel', opt.textContent.length > 8 ? opt.textContent.substring(0,8)+'…' : opt.textContent);
      closeRoleDropdown();
      loadDownline();
    });
  });
  ...
  async function loadDownline(searchId = '') {
    ...
    if (error || !data || data.length === 0) {
      empty.style.display = 'flex'; tableWrap.style.display = 'none'; return;
    }
  ...
  document.getElementById("langBtn").addEventListener("click", () => {
    lang = lang === 'my' ? 'en' : 'my';
    setEl('langLabel', lang === 'my' ? 'မြန်မာ' : 'EN');
  });
  ...
  document.getElementById('registerBtn').addEventListener('click', async () => {
    ...
    try {
      const resp = await fetch("https://xjqrwcsxiaybpztzestb.supabase.co/functions/v1/register-user",{method:"POST",headers:{"Content-Type":"application/json","apikey":supabaseKey,"Authorization":"Bearer "+supabaseKey},body:JSON.stringify({phone,password,fullname:name,referrer_code:refCode||null})});
      const result = await resp.json();
      if (resp.ok) { alert("မှတ်ပုံတင်ခြင်း အောင်မြင်သည်!\nReferral Code: "+result.ref_code); modal.classList.remove('active'); onLoginSuccess({phone,name},result.ref_code,0,null); }
  ...
  function onLoginSuccess(user, refCode, balance=0, userId=null) {
    ...
    // Unlock spin button (assume 1 daily spin on login)
    availableSpins = 1;
    setEl('availableSpins', availableSpins);
    ...
    if (ticker && refCode) {
      const t = ` &rsaquo; Agent ID: ${refCode} &nbsp;&nbsp;&nbsp; &rsaquo; Diamond-BETT Affiliate &nbsp;&nbsp;&nbsp;`;
